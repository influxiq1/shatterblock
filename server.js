        ...
        \  <span style="display: block;  font-size: 16px; text-transform: capitalize; color: #dcdcdc; font-weight: bold; line-height: 29px;">\
		 \  <span style="display: inline-block; width:100%;">UserID:'+items[0]._id+'</span>\
             \  <span style="display: inline-block; width:100%;">username:'+items[0].username+'</span>\
               \</span>\
                \ <a  href="https://sol.apogeehost.com/~zufelt/password_reset?link_key=uziD4FoxOIracHmYPOQk" style="display: block; text-decoration: none; background:#faa318 ;  color: #fff; width: 80%; padding: 10px 2px; display: block; margin: 0 auto; margin-top: 10px; font-weight: bold; font-size: 18px;">Please follow this link to reset your password:<span style="text-align: center; font-size: 12px; width: 100%; display: inline-block; color: #111111; word-break: normal; word-wrap: normal; overflow: hidden;margin-top: 4px;">https://sol.apogeehost.com/~zufelt/password_reset?link_key=uziD4FoxOIracHmYPOQk</span>\</a>\
          ...
          var x=sgMail.send(msg);
           /* resp.send(JSON.stringify({'status': 'success', 'item': items,'result':result,'result1': result[0].apikey,'x':x}));*/
...
app.post('/sendforgotpasswordemail1', function (req,resp){
    ...
    /*collection.find({email:req.body.email}).toArray(function(err, re) {
        if(err){
            resp.send(JSON.stringify({'status':'error'}));
        }
        else{
           /!* resp.send(JSON.stringify({'status':'success','resultofnewfieldgenaretion':re}));*!/
           collection.insert( [{temp_forgetpass_key:params }]);
        }
    })*/
});
function makeid( ln) {
